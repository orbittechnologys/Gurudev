...
function convertTextArea(textAreaId) {

    console.log($(textAreaId))
    let editor = $(textAreaId).siblings('#cke_content_new')
    editorDiv = $(editor).children()[1]
    let iframe = $(editorDiv).find('#cke_1_contents').children()[1]
    iframeBody = $(iframe).contents().find("body")
    recursiveEach(iframeBody);
    ...
    function recursiveEach(element) {
        ...
        $(element).contents().each(function() {
            ...
            if (this.nodeType == 1) {
                ...
                let pureText = this.data;
                var words = pureText.split(' ');
                ...
                this.data = op_words.join(' ')
            }
        });
    }
}

function convertQuizTextArea(textAreaId) {


    let editor = $("#" + textAreaId).siblings('#cke_' + textAreaId)
    editorDiv = $(editor).children()[1]
    let iframe = $(editorDiv).find('.cke_contents').children()[1]
    iframeBody = $(iframe).contents().find("body")
    recursiveEach(iframeBody);
...
function bind_events() {
    $("#convert-current-affairs").click(function() {
        convertTextArea('#content_new');
        $("#title").val(convertText($('#title').val()));
        $("#tags").val(convertText($('#tags').val()));
        $("#source").val(convertText($('#source').val()));
    });
   $('body').on('click', '.to-kannada', function(e) {
        let textAreaId = $(this).attr('editor-id')
        let descriptionId = $(this).attr('description-id')
        convertQuizTextArea(textAreaId);
        if (descriptionId != '') {
            $('#' + descriptionId).val(convertText($('#' + descriptionId).val()));
        }
    });
}
