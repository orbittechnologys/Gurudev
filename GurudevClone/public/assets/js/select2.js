...
$(document).ready(function() {
	...
	$('.select2-show-search').select2({
		...
		if(isNew.length && newVal===selectedval){
			var Model=$(this).attr('dataModel')
			var url=$('#addNewMaster').text();
			var data={'model':Model,'fieldValue':newVal,'fieldName':$(this).attr('dataFieldName'),'dataReturnId':$(this).attr('dataReturnId')}
			$.ajax({
				...
				success: function (data) {
					if(data['status']) {
						isNew.replaceWith('<option selected value="' + data['value'] + '">' + data['text'] + '</option>');
