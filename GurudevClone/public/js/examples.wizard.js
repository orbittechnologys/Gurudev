...
(function($) {
	...
	*/
	...
	$('#w4').find('ul.pager li.previous').on('click',function(e){
		e.preventDefault()
	});
	var previousClick=0;
		$('#w4').bootstrapWizard({
		...
		onTabShow: function( tab, navigation, index) {
			var $total = navigation.find('li').length - 1;
			var $current = index;
			var is_check= $('input[name="is_hostler"]:checked').val();
			var is_trans_check= $('input[name="is_transportation"]:checked').val();
			var hostelIndex=5;var transportationIndex=6;
			if($('.course_id').val()==2){
                ++hostelIndex;
                ++transportationIndex;
            }

			if(index==hostelIndex &&  is_check==0 ){
			    if(previousClick==0){
                    $current=++hostelIndex;
                    $('#w4').bootstrapWizard('next');
                } else if(previousClick==1){
                    $current=--hostelIndex;
                    $('#w4').bootstrapWizard('previous');
                }
				return false
			}
			if(index==transportationIndex &&  is_trans_check==0){
                if(previousClick==0) {
                    $current = ++transportationIndex;
                    $('#w4').bootstrapWizard('next')
                }
                else if(previousClick==1){
                    $current=--hostelIndex;
